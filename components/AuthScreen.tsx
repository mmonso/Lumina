...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin, onRegister, theme, onThemeChange, isLoading }) => {
  ...
  const inputClass = `w-full px-4 py-3 rounded-xl outline-none transition-all duration-300 backdrop-blur-md ${
    theme.isDark 
      ...
      <div className={`w-full max-w-md p-8 rounded-3xl shadow-2xl backdrop-blur-2xl border animate-fade-in-up ${theme.isDark ? 'bg-black/40 border-white/10' : 'bg-white/60 border-white/40'}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full py-3.5 rounded-xl font-medium tracking-wide shadow-lg transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${
              theme.isDark
        ...
        <div className="mt-8 text-center">
          <button
            ...
            className={`text-xs font-medium tracking-wider uppercase transition-colors hover:underline ${theme.isDark ? 'text-white/60 hover:text-white' : 'text-slate-500 hover:text-slate-800'}`}
          >
